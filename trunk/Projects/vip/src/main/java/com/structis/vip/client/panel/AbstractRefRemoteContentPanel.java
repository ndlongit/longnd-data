 ...
 * @param <M>
 */
public abstract class AbstractRefRemoteContentPanel<M extends BaseModelDataActivable>
	...
	protected void onLoadApplication(Map<String, String> params) {
		...
		pagingLoadConfig.setLimit(pagingSize);
	 ...
	 * @param params
	 */
	private void addSearch(ReferencePagingLoadConfig pagingLoadConfig,
	 ...
	 * @return
	 */
	private void addParamSearch(ReferencePagingLoadConfig pagingLoadConfig,
			Map<String, String> params){
		...
		DataProxy proxy = new RpcProxy<PagingLoadResult<M>>() {   
			...
			protected void load(Object loadConfig,
				...
				if (loadConfig instanceof ReferencePagingLoadConfig){
					...
					pagingLoadConfig.setRechercheProperties(searchProperties);
				}
				else {
				...
				if (null == pagingLoadConfig.getSortField()) {
					pagingLoadConfig.setSortFields(getDefaultSort());
				...
				if (pagingLoadConfig.getRechercheProperties() != null) {
					addParamSearch(pagingLoadConfig, map);
				}
				HistoryHelper.newItem(navigation.getActionActuelle().getLabel(), map, false);
				...
				loadPaging(pagingLoadConfig, callDecorated);
			}   
		 };
		return proxy;
	}
	...
	protected String[] getDefaultSort() {
		return new String [] {BaseModelDataActivable.BASE_LIBELLE};
	}
