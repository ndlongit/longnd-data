...
/**
 * Class "mère" des Vues, permettant de centraliser des mécanismes communs aux vues quelque soit leur type...
 * 
 * @author th.bernard
 * 
 * @param <C>
...
public abstract class AbstractView<C extends LayoutContainer> extends BaseObservable {
    ...
    public void init(final List datas, final boolean load, final boolean clearChild) {
        ...
        if (!this.init) {
            ...
            if (clearChild) {
                ...
                this.clearChild(this.mainContainer);
            }
        }
        if (load) {
            GWT.log(viewClassName + ".load (from AbstractView)");
            this.load(this.mainContainer, datas);
        }
    ...
    public void addChild(AbstractView<LayoutContainer> view) {
        ...
        this.addChildContent(this.mainContainer, view);
    }
    ...
    /**
     * Méthode permtettant de lancer les événements propre à l'application depuis les vues (Observables)
     */
     ...
     * 
     * @param url
     *            L'url qui remplacera l'actuelle
     */
     ...
     * 
     * @param url
     *            L'url a ouvrir dans un nouvel onglet ou un nouveau navigateur
     */
