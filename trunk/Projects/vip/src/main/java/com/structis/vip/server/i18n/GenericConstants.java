...
public class GenericConstants extends GenericX {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        if (method.getParameterTypes().length == 1 && isA(method.getParameterTypes()[0], String.class) && args.length == 1
                && args[0] instanceof String && this.isMethodOf(method, ConstantsWithLookup.class)) {
            return this.invokeGenericMethod(proxy, (String) args[0], method.getReturnType());
        ...
        Object result = null;
        if (k != null) {
            result = this.buildConstant(k.value(), method);
        }
        if (result == null) {
            result = this.buildConstant(method.getName(), method);
        }
        return result;
    }
    ...
    private Object invokeGenericMethod(Object proxy, String methodName, Class<?> returnType) throws Throwable {
        try {
            ...
            return this.itf.getMethod(methodName, new Class<?>[0]).invoke(proxy, new Object[0]);
        } catch (Throwable e) {
            // use the method name as property name
            String resultAsString = this.properties.getProperty(methodName);
            if (resultAsString == null) {
                return null;
            } else {
                // Convert string value under the required Type
                return this.toObject(resultAsString, returnType);
            }
        }
    }

    private Object buildConstant(String propertyName, Method method) {
        String resultAsString = this.properties.getProperty(propertyName);
        if (resultAsString == null) {
            // use the default value (annotation) if there is one
            return this.getDefaultValue(method);
        } else {
            // Convert string value under the required Type
            return this.toObject(resultAsString, method.getReturnType());
        }
    }
    ...
    private Object getDefaultValue(Method method) {
        ...
        } else if (this.isStringMap(returnClass)) {
        ...
        } else if (this.isStringArray(returnClass)) {
    ...
    private Object toObject(String resultAsString, Class<?> clazz) {
        ...
        } else if (this.isStringMap(clazz)) {
            Map<String, String> result = new HashMap<String, String>();
        ...
        } else if (this.isStringArray(clazz)) {
