...
public class PerimetreTypeListPanel extends LayoutContainer {
...
//		if (ConstantClient.ENTITE_ID_IS_BYEFE.equals(SessionServiceImpl.getInstance().getEntiteContext().getEntId())) {
		...
		clientPerimetreTypeService.getPerimetreTypes(SessionServiceImpl.getInstance().getEntiteContext().getEntId(), new AsyncCallback<List<PerimetreTypeModel>>() {
					@Override
	...
	private void initUI() {
		...
		GridCellRenderer<PerimetreTypeModel> isSubdelegableRenderer = new GridCellRenderer<PerimetreTypeModel>() {
			@Override
			public Object render(final PerimetreTypeModel model, String property,
