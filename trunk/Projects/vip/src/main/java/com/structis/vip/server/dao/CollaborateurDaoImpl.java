...
public class CollaborateurDaoImpl extends HibernateGenericDao<Collaborateur, Integer> implements CollaborateurDao {
    ...
    public List<Collaborateur> getAllCollaborateursByEntiteId(String entiteId, Boolean sortie) {
        ...
        sb.append(" from Collaborateur c where c.entite.entId = :entiteId").append(subSortie).append(" order by c.lastname, c.firstname");
    ...
    public List<Collaborateur> findByName(String name, String entityId, Boolean sortie) {
        // String sql =
        // " FROM Collaborateur c WHERE c.lastname LIKE :name AND c.entite.entId = :entityId AND (c.dateSortie is null or c.dateSortie > GETDATE()) ORDER BY c.lastname, c.firstname";
        String subSortie = "";
        if (!sortie) {
            subSortie = " AND ( c.sorti is null or c.sorti = 'N' or ( c.sorti = 'O' AND (c.dateSortie is null or c.dateSortie >= GETDATE())))";
        }
        String sql = " FROM Collaborateur c WHERE c.lastname + ', ' + c.firstname LIKE :name AND c.entite.entId = :entityId " + subSortie
    ...
    public List<Collaborateur> findByName(String name, String entityId, Boolean sortie, String sortedField, Integer sortDir, Integer start,
        ...
        sql.append("select d.* from ( select ROW_NUMBER() OVER (")
                ...
                .append(" FROM COL_COLLABORATEUR c WHERE c.col_lastname + ', ' + c.col_firstname LIKE :name AND c.ent_id = :entityId " + subSortie
                        + ") as d ").append(" WHERE d.RowNum BETWEEN :startOffset AND :endOffset");
    ...
    public List<Collaborateur> getAllDelegantsByPerimeter(String perId, String entiteId) {
        // String sql =
        // " FROM Collaborateur c where (c.perimetreDelegant.perId = :perId OR c.perimetreDelegant.perId is null) and c.isDelegant = 1 and c.entite.entId = :entiteId";
        String sql = " select distinct c.delegant FROM DelegantPerimetre c inner join c.delegant where (c.perimetre.perId = :perId OR c.perimetre.perId = '"
                + this.getRootPerimetre(entiteId) + "') and c.delegant.isDelegant = 1 and c.delegant.entite.entId = :entiteId";
        // String sql =
        // " FROM Collaborateur c where (c.perimetreDelegant.perId = :perId OR c.perimetreDelegant.perId is null OR c.perimetreDelegant.perId = '"+getRootPerimetre(entiteId)+"') and c.isDelegant = 1 and c.entite.entId = :entiteId";
        Query query = this.getEntityManager().createQuery(sql);
    ...
    @Override
    public List<KeyValueVM> getAllKeyValueCollaborateursByEntiteId(String entiteId, boolean sortie) {
        StringBuffer sb = new StringBuffer();
        String subSortie = "";
        ...
        sb.append(
                " select new com.structis.vip.server.bean.domain.core.business.KeyValueVM(c.firstname, c.lastname + ', '+ c.firstname,'0') from Collaborateur c where c.entite.entId = :entiteId")
                .append(subSortie).append(" order by c.lastname, c.firstname");
    ...
    public Long countByName(String name, String entId, Boolean sortie) {
        ...
        String sql = " select count(*) FROM Collaborateur c WHERE c.lastname + ', ' + c.firstname LIKE :name AND c.entite.entId = :entityId "
                + subSortie;
    ...
    public Long countAllCollaborateursByEntiteId(String entId, Boolean sortie) {
        ...
        sb.append(" select count(*) from Collaborateur c where c.entite.entId = :entiteId").append(subSortie);
    ...
    public String getAllParentPerimetreAndCurrent(String entId, String perId) {
        Query query = this.getEntityManager().createNativeQuery("select dbo.ufn_GetAllParentsAndCurrent(?,?) as parents");
    ...
    // alias = "";
    // }
    // subSortie = " AND ( " + alias + "sorti is null or " + alias + "sorti = 'N' or ( " + alias + "sorti = 'O' AND (" + alias +
    // "dateSortie is null or " + alias + "dateSortie >= GETDATE())))";
    // }
    // return subSortie;
    // }
