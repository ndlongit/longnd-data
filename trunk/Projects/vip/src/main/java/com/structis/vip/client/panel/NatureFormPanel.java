...
public class NatureFormPanel extends LayoutContainer {
	...
	private void save() {
		...
		if (isEdit == false) {
			clientDelegationNatureService.insert(model, new AsyncCallback<DelegationNatureModel>() {
			...
			clientDelegationNatureService.update(model, new AsyncCallback<DelegationNatureModel>() {
				@Override
				public void onSuccess(DelegationNatureModel arg0) {
					ContentEvent contentEvent = new ContentEvent();
					contentEvent.setMode(ContentEvent.CHANGE_MODE_TO_ADMIN_NATURE_LIST);
