...
public class DomDelegationServiceImpl extends GenericEntityServiceImpl<Delegation, Integer> implements
		DomDelegationService {
	...
	public Delegation insert(Delegation delegation) {
		Delegation dl = delegationDao.insert(delegation);		
		return dl;
	}
	...
	public List<Delegation> getValidDelegations(String perId, Integer natureId,
		...
		return delegationDao.getValidDelegations(perId, natureId, typeId, statusId, delegantId,
				delegataireId, startDate, endDate, sep, conjointe, isDisplayAllLevel);
	}	
	...
	public List<Delegation> getValidDelegationsByEntite(String enId, String perId, Integer natureId,
			...
			Integer delegataireId, Date startDate, Date endDate,
			Boolean sep, Boolean conjointe, Boolean isDisplayAllLevel) {
		return delegationDao.getValidDelegations(enId, perId, natureId, typeId, statusId, delegantId,
				delegataireId, startDate, endDate, sep, conjointe, isDisplayAllLevel);
	}	
	
	@Override
	public List<Delegation> getValidDelegationsByEntite(String enId, String perId, List<Integer> natureIds,
			...
			Boolean sep, Boolean conjointe, Boolean isDisplayAllLevel) {		
		return delegationDao.getValidDelegations(enId, perId, natureIds, typeIds, statusIds, delegantIds,
				delegataireIds, startDate, endDate, sep, conjointe, isDisplayAllLevel);
	}	
		
	@Transactional
	public Boolean update(Delegation dl) {				
		delegationDao.update(dl);
		return true;
	}
	@Override
	...
	public Boolean hasDelegation(Integer natureId, String perimetreId) {		
		return delegationDao.hasDelegation(natureId, perimetreId);
	}
	...
	public Boolean hasOtherDelegation(Integer delId, Integer dnId, String perId, String enId) {
		...
		boolean isMultiDelegation = delegationModelDao.hasMultipleDelegation(dnId, enId, perId);
		if (isMultiDelegation) return false;
		return delegationDao.hasOtherDelegation(delId, dnId, perId, enId);
	}

	@Override
	public Boolean delegantIsDelegataireOfDelegation(Integer delegantId,
			Integer delegationId) {
		return delegationDao.delegantIsDelegataireOfDelegation(delegantId,
				delegationId);
	}

	@Override
	public Integer subDelegantIsDelegataireOfParent(Integer delId) {
		return delegationDao.subDelegantIsDelegataireOfParent(delId);
	}
	...
	public List<Delegation> getDelegationsByEntite(String enId, String perimetreId,
			...
			List<Integer> delegataireIds, Date startDate, Date endDate, Boolean sep,
			Boolean conjointe, Boolean isDisplayAllLevel, PerimetreTreeModel perimetreTreeModel, List<UserRoleModel> userRoles) {
		return delegationDao.getDelegations(false, enId, perimetreId, natureIds,
	...
	public List<Delegation> getAllDelegationById(List<Integer> ids) {
		return delegationDao.getAllDelegationById(ids);
	}
	...
	public Delegation renewDelegation(Delegation delegation) {		
		...
		delegationDao.update(delegation);
			...
			return delegationDao.insert(newDel);			
		
	}

	@Override
	public List<Delegation> updateStatus() {
		List<Delegation> delegations = delegationDao.getDelegationWithStatus(DelegationConstants.DEL_STATUS_VENIR);
	...
	public Delegation insert(Delegation delegation,
		...
		if (!dl.getEntite().getEntId().equals(Constants.ENTITE_ID_ETDE) && dds != null) {
			for (DelegationDelegataireModel d: dds) {
				...
				delegationDelegataireDao.insert(d1);
			}
		}
		return dl;
	}
	...
	public boolean update(Delegation delegation,
		...
		if (!delegation.getEntite().getEntId().equals(Constants.ENTITE_ID_ETDE) && dds != null) {
			delegationDelegataireDao.deleteByDelegation(delegation.getId());
			for (DelegationDelegataireModel d: dds) {
				DelegationDelegataire d1 = new DelegationDelegataire();
				d1.setColId(d.getColId());
	...
	public String findDelegataireNames(Integer delId) {
		return delegationDao.findDelegataireNames(delId);
	}

	@Override
	public List<Collaborateur> findDelegatairesByDelegation(int delId) {
		return delegationDelegataireDao.findDelegatairesByDelegation(delId);
	}
}
