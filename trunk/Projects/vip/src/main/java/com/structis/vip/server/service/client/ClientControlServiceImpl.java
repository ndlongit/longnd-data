...
public class ClientControlServiceImpl extends DependencyInjectionRemoteServiceServlet implements
	...
	public Boolean delete(ControlModel model) {
		Control dm = (Control) modelBeanMapper.map(model);
	...
	public ControlModel insert(ControlModel model) {		
		...
		if (eccs != null && eccs.size() > 0) {
			clientExtControlService.insert(doc.getId(), eccs);
		}
	...
	public List<ControlModel> findAll() {
		...
		return (List<ControlModel>) callManager(callBack);
	}

	@Override
	public PagingLoadResult<ControlModel> getControlsWithPaging(
			ControlFilter config) {
		List<ControlModel> all = new ArrayList<ControlModel>();
		all = getValidControls(config);
...
//	    	model.removeUnusedOnList();
	...
	public List<ControlModel> getControls(
			ControlFilter config) {
		List<ControlModel> all = getValidControls(config);
		return all;
	...
	private List<ControlModel> getValidControls(final ControlFilter filter) {
		ManagerCallBack callBack = new ManagerCallBack() {
			public Object execute(Object... inputs) {
				...
				List<Control> ret = domControlService.getControlsByEntite(enId, perimetreId, getKeyList(filter.getTypes()), 
						filter.getStartDate(), filter.getEndDate(), filter.getCodeProject(), getKeys(filter.getCaracteres()), filter.getControllerName(), filter.getPerimetreTreeModel(), filter.getUserRoles());
				for (Control c: ret) {					
		...
		List<ControlModel> rets = (List<ControlModel>) callManager(callBack);
