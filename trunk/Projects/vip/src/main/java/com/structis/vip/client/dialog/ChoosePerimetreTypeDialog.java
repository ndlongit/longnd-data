...
public class ChoosePerimetreTypeDialog extends Window {
	...
	public void initData(final String parentId) {
		...
		clientPerimetreTypeServiceAsync.getPerimetreTypes(ConstantClient.ENTITE_ID_IS_ETDE,
				new AsyncCallback<List<PerimetreTypeModel>>() {
					...
					public void onSuccess(List<PerimetreTypeModel> arg0) {
						if (parentId != null) {
							...
							clientPerimetreServiceAsync.sync(ConstantClient.ENTITE_ID_IS_ETDE, parentId, arg0,
									new AsyncCallback<Map<String, List<PerimetreModel>>>() {
										...
										public void onSuccess(Map<String, List<PerimetreModel>> arg0) {
											...
											perimetres.add(arg0.get(Constants.SUCCESS_LIST));
											Info.display(messages.commoninfo(),
													messages.perimetretypechoosesyncsuccess() + " " + arg0.get(Constants.SUCCESS_LIST).size() + " "
										...
										private void displayErrorSyncList(
												List<PerimetreModel> errorList) {											
										...
										public void onFailure(Throwable arg0) {
											Info.display(messages.commoninfo(),
													messages.perimetretypechoosesyncfailed());
											perimetreGrid.unmask();
											btnCancel.setEnabled(true);
										}
									});
