...
public class StringUtils {
	public static String htmlize(String text) {
		return text.startsWith("<?") || text.startsWith("<html>") ? text : "<html><body>" + text.replace("&", "&amp;").replace(
				"<", "&lt;").replace(">", "&gt;").replace("\n", "<br>") + "</body></html>";
	}
	...
	/*
	 * public static final String implode(Object[] strings, String separator) { return implode(Arrays.asList(strings),
	 * separator); }
	 */
	...
	/*
	 * public static final String implode(Collection<?> strings, String separator) { int size = 0, n = strings.size(); for
	 * (Object s : strings) if (s != null) size += s.toString().length(); StringBuffer out = new StringBuffer(size +
	 * separator.length() * (n == 0 ? 0 : n - 1)); boolean first = true; for (Object s : strings) { if (s == null) continue;
	 * if (first) first = false; else out.append(separator); out.append(s); } return out.toString(); }
	 */
					...
					case 'u' :
						try {
							...
							throw new IllegalArgumentException(
									"Illegal unicode escaping in string \"" + s + "\" at index " + i, ex);
						}
						break;
					default :
