...
public class AdminPanel extends AbstractPanel {
	...
	public AdminPanel() {
		...
		adminFieldset.setWidth(GuiUtil.getScreenWidth() / 2 - 30);
		FieldSet chantierFieldset = CreateChantierFieldset();
		chantierFieldset.setWidth(GuiUtil.getScreenWidth() / 2 - 30);
				...
				Listener<MessageBoxEvent> callback = new Listener<MessageBoxEvent>() {
					...
					public void handleEvent(MessageBoxEvent be) {
						...
						if (Dialog.OK.equals(btt.getItemId())) {
							...
							service.updateListUser(listUserEnd, listUserAdminDelete, listUserInChantierAfter, listUserDeleteInChantier, idChantier,
	...
	public FieldSet createAdminFieldSet() {
		...
		column_1.setRenderer(new GridCellRenderer<UtilisateurGrpModel>() {
			...
			@Override
			public Object render(UtilisateurGrpModel model, String property, ColumnData config, int rowIndex, int colIndex,
					ListStore<UtilisateurGrpModel> store, Grid<UtilisateurGrpModel> grid) {
		...
		txtAdmin.setBorders(true);
		txtAdmin.setStyleAttribute("border-color", "#938E8E");
		txtAdmin.addKeyListener(new KeyListener() {

			@Override
			public void componentKeyUp(ComponentEvent event) {
				if (txtAdmin.getValue() != null && !txtAdmin.getValue().trim().isEmpty()
						&& txtAdmin.getValue().toLowerCase().matches("\\b.{0,60}\\\\+.{1,60}")) {
	...
	public FieldSet CreateChantierFieldset() {
		...
		LabelField lblfldNouveauxAdministrateur = new LabelField("Nouveau  :");
		layoutContainer.add(lblfldNouveauxAdministrateur);
		layoutContainer.add(new Html("&nbsp"));
		...
		txtChantier.addKeyListener(new KeyListener() {
			...
			public void componentKeyUp(ComponentEvent event) {
				if (txtChantier.getValue() != null && !txtChantier.getValue().trim().isEmpty()
						&& txtChantier.getValue().toLowerCase().matches("\\b.{0,60}\\\\+.{1,60}")) {
		...
		cbbChantier.setStyleAttribute("marginLeft", "25px");
		...
		cbbChantier.setBorders(true);
		cbbChantier.setStyleAttribute("border-color", "#938E8E");
	...
	public GridCellRenderer<UtilisateurGrpModel> deleteButton() {
		GridCellRenderer<UtilisateurGrpModel> deleteBtn = new GridCellRenderer<UtilisateurGrpModel>() {
			...
			@Override
			public Object render(final UtilisateurGrpModel model, String property, ColumnData config, int rowIndex, int colIndex,
					ListStore<UtilisateurGrpModel> store, Grid<UtilisateurGrpModel> grid) {
