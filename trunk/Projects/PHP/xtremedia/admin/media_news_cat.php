...
$edit_url = 'index.php?act=news_cat&mode=edit';
...
$inp_arr = array(
		...
		'sub'	=> array(
			...
			'type'	=>	'function::acp_news_maincat::number'
		),
	);
##################################################
# ADD NEWS CAT
...
if ($mode == 'edit') {
	...
	if ($cat_id) {
		if (!$_POST['submit']) {
			$q = $mysql->query("SELECT * FROM ".$tb_prefix."news_cat WHERE cat_id = '$cat_id'");
			...
			$error_arr = $form->checkForm($inp_arr);
			if (!$error_arr) {
				$sql = $form->createSQL(array('UPDATE',$tb_prefix.'news_cat','cat_id','cat_id'),$inp_arr);
				eval('$mysql->query("'.$sql.'");');
		...
		if ($_POST['sbm']) {
			$z = array_keys($_POST);
			$q = $mysql->query("SELECT cat_id FROM ".$tb_prefix."news_cat");
			for ($i=0;$i<$mysql->num_rows($q);$i++) {
				$id = split('o',$z[$i]);
				$od = ${$z[$i]};
				$mysql->query("UPDATE ".$tb_prefix."news_cat SET cat_order = '$od' WHERE cat_id = '".$id[1]."'");
			}
		}
		echo "<script>function check_del(id) {".
		"if (confirm('Bạn có muốn xóa thể loại này không ?')) location='?act=news_cat&mode=del&cat_id='+id;".
		"return false;}</script>";
		...
		$cat_query = $mysql->query("SELECT * FROM ".$tb_prefix."news_cat WHERE (sub_id IS NULL OR sub_id = 0) ORDER BY cat_order ASC");
		while ($cat = $mysql->fetch_array($cat_query)) {
			...
			$sub_query = $mysql->query("SELECT * FROM ".$tb_prefix."news_cat WHERE sub_id = '".$cat['cat_id']."' ORDER BY cat_order ASC");
...
if ($mode == 'del') {
	...
	if ($cat_id) {
		if ($_POST['submit']) {
			$mysql->query("DELETE FROM ".$tb_prefix."news_cat WHERE cat_id = '".$cat_id."'");
