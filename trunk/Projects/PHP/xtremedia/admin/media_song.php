...
$inp_arr = array(
	...
	/*	'more_url'			=> array(
			...
			'name'		=>	'More URL',
			'type'		=>	'url_more',
			'desc'	=>	'Dùng khi Post phim, nếu phim được chia ra nhiều File thì Post các link vào đây ( Đầy đủ http://.../filename.wmv )<br>Còn mục Link ở trên thì nên Post đoạn Trailer thôi.',
			'can_be_empty'	=>	1
		), */
		'public'	=> array(
			'table'	=>	'm_public',
			...
			'type'	=>	'checkbox',
			'checked'	=>	false,
			'desc'	=>	' Có thể Download mà không cần đăng ký thành viên.',
			'can_be_empty'	=>	true,
		),
		'lyric'			=> array(
			'table'		=>	'm_lyric',
...
if ($mode == 'edit') {
	...
	elseif ($m_id) {
		...
		if ($mysql->num_rows($q)) {
			...
			echo "<tr align=center><td width=3%><input class=checkbox type=checkbox name=chkall id=chkall onclick=docheck(document.media_list.chkall.checked,0) value=checkall></td><td class=title width=60%>Tên Media</td><td class=title>Ca sĩ</td><td class=title>Dạng File</td><td class=title>Lỗi</td><td class=title>Play</td></tr>";
			while ($r = $mysql->fetch_array($q)) {
				$id = $r['m_id'];
				$title = $r['m_title'];
				switch ($r['m_type']) {
					case 1 : $file_type = 'AUDIO'; break;
					case 2 : $file_type = 'FLASH'; break;
					case 3 : $file_type = 'MOVIE'; break;
					case 4 : $file_type = 'EBOOK'; break;
					case 5 : $file_type = 'APPLICATION'; break;
					case 6 : $file_type = 'ARCHIVE'; break;
				}				
				$singer = $r['m_singer'];
				$broken = ($r['m_is_broken'])?'<font color=red><b>X</b></font>':'';
				echo "<tr><td><input class=checkbox type=checkbox id=checkbox onclick=docheckone() name=checkbox[] value=$id></td><td class=fr><a href='$link&m_id=".$id."'><b>".$title."</b></a></td><td class=fr_2 align=center><b><a href=?act=singer&mode=edit&singer_id=".$singer.">".m_get_data('SINGER',$singer)."</a></b><td align=center class=fr>".$file_type."</td></td><td class=fr align=center>".$broken."</td><td class=fr align=center><a href='../#Play,".$id."' target=_blank >Play</a> </td></tr>";
			}
			echo "<tr><td colspan=6>".admin_viewpages($tt,$m_per_page,$pg)."</td></tr>";
			echo '<tr><td colspan=6 align="center">Với những Media đã chọn : '.
