	...
	*/
	function parseXML($xmlText, $useSAXY = true, $preserveCDATA = true, $fireLoadEvent = false) {
		...
		if (DOMIT_Utilities::validateXML($xmlText)) {
			...
			//domit hacks, expat parser overhauled in php 5
			//force php 5 to use saxy
			if ($useSAXY || (!function_exists('xml_parser_create')) || version_compare(phpversion(), '5.0', '>=')) {
