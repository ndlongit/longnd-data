...
(function($){
					...
					if (data.query) {
						...
						if (data.query.results.channel.length > 0 ) {
							...
							for (var i=0; i<result; i++) {
								...
								_callback(e, data.query.results.channel[i], options,locations[i]);
							}
						} else {
	...
	var _callback = function(e, feed, options,locationid) {
		var $e = $(e);
		...
		if (options.link) {
			html += '<div class="weatherLink"><a href="http://uk.weather.com/weather/10day-'+ locationid +'" target="'+ options.linktarget +'" title="Read full forecast">10 day forecast</a></div>';
		}
