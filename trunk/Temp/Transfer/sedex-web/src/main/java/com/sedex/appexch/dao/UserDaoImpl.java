...
public class UserDaoImpl extends AbstractDao<User, Long> implements UserDao {
	...
	public User getByLoginName(String loginName) {
		User result = super.findUniqueByProperty(User.PROP_LOGIN_NAME,
				loginName);
		return result;
	}
	...
	public List<User> searchJpa(UserSearch searchModel) {
		...
		if (user != null) {
			...
			addLikeExpression(userConditions, cb,
					rootEntity.get(User.PROP_LOGIN_NAME), user.getLoginName());
			addLikeExpression(userConditions, cb,
					rootEntity.get(User.PROP_LAST_NAME), user.getLastName());
			addLikeExpression(userConditions, cb,
					rootEntity.get(User.PROP_FIRST_NAME), user.getFirstName());
			addLikeExpression(userConditions, cb,
					rootEntity.get(User.PROP_EMAIL), user.getEmail());
		}
		...
		if (group != null) {
			...
			if (!isNullOrEmpty(Arrays.asList(groupName, groupCode))) {
				...
				Join groupJoin = rootEntity.join(User.PROP_GROUPS,
						JoinType.LEFT);
				if (!isNullOrEmpty(groupName)) {
					groupConditions.add(buildLikeExpression(cb,
							groupJoin.get(Group.PROP_NAME), groupName));
				}
				if (!isNullOrEmpty(groupCode)) {
					groupConditions.add(buildLikeExpression(cb,
							groupJoin.get(Group.PROP_CODE), groupCode));
				}
			}
		}
	...
	public List<User> searchHibernate(UserSearch searchModel) {
		...
		if (user != null) {
			...
			if (!isNullOrEmpty(loginName)) {
				cr.add(Restrictions.like(User.PROP_LOGIN_NAME,
						"%" + loginName.trim() + "%"));
			}

			String firstName = user.getFirstName();
			if (!isNullOrEmpty(firstName)) {
				cr.add(Restrictions.like(User.PROP_FIRST_NAME,
						"%" + firstName.trim() + "%"));
			}
		}
		...
		if (group != null) {
			String groupName = group.getName();
			if (!isNullOrEmpty(groupName)) {
				cr.add(Restrictions.like("group." + Group.PROP_NAME, "%"
						+ groupName.trim() + "%"));
			}
			String groupCode = group.getCode();
			if (!isNullOrEmpty(groupCode)) {
				cr.add(Restrictions.like("group." + Group.PROP_CODE, "%"
						+ groupCode.trim() + "%"));
			}
		}

		// ProjectionList columns =
		// Projections.projectionList().add(Projections.property(User.PROP_ID),
		// "user.id")
