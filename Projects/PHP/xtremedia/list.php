...
$fields = "m_poster, m_id, m_title, m_singer, m_type, m_viewed, m_downloaded, IF(m_lyric = '' OR m_lyric IS NULL,0,1) m_lyric";
$q = '';
if ($value[0] == 'List') {
...
if ($mysql->num_rows($q)) {
	...
	elseif ($value[0] == 'Home') $cat_tit = 'Danh sách các ca khúc';
	...
	while ($r = $mysql->fetch_array($q)) {
		...
		$poster = m_get_data('USER',$r['m_poster']);
		switch ($r['m_type']) {
		...
		$html .= $tpl->assign_vars($t['row'],
			array(
				...
				'song.TITLE' => getwords($r['m_title'],6),
