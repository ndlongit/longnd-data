...
function acp_singer_type($i) {
	$html = "<select name=singer_type>".
		...
		"<option value=9".(($i==9)?' selected':'').">Thành viên tự thể hiện</option>".
	"</select>";
	return $html;
}
...
function acp_news_cat($id = 0, $add = false) {
	global $mysql,$tb_prefix;
	$q = $mysql->query("SELECT * FROM ".$tb_prefix."news_cat ORDER BY sub_id ASC");
	while ($r = $mysql->fetch_array($q)) {
		//echo $r['cat_id'].'-'.$r['sub_id'].'<br>';
		if (!$r['sub_id']) $arr[$r['cat_id']][0] = $r['cat_name'];
		elseif ($arr[$r['sub_id']][0]) $arr[$r['sub_id']][] = array($r['cat_id'],$r['cat_name']);
...
function acp_type(&$url) {
	...
	elseif (in_array($ext,$ebook_arr)) $type = 4;
	elseif (in_array($ext,$app_arr)) $type = 5;
	elseif (in_array($ext,$archive_arr)) $type = 6;
//	elseif (in_array($ext,$others_arr)) $type = 8;
	elseif (!$type) $type = 1;
...
function acp_news_maincat($id) {
	global $mysql,$tb_prefix;
	$q = $mysql->query("SELECT * FROM ".$tb_prefix."news_cat WHERE sub_id = 0 OR sub_id IS NULL ORDER BY cat_order ASC");
	$html = "<select name=sub>";
	$html .= "<option value=0>- Mục chính -</option>";
	while ($r = $mysql->fetch_array($q)) {
		$html .= "<option value=".$r['cat_id'].(($r['cat_id'] == $id)?" selected":"").">".$r['cat_name']."</option>";
...
function acp_get_mod_permission() {
	...
	$permission_list = array(
		...
		'add_album',	'edit_album',	'del_album',
