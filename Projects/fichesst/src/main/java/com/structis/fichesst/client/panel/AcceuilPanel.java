...
public class AcceuilPanel extends AbstractPanel {
		...
		SelectionListener<ButtonEvent> add = new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				...
				if (isExistIdentifiant(txtChantier.getValue())) {
					service.createChantier(chantier, user.getId(), new AsyncCallbackWithErrorResolution<Boolean>() {
						@Override
	...
	public void loadChantier() {
		serviceUser.checkUserByKerberosSSO(new AsyncCallbackWithErrorResolution<UtilisateurGrpModel>() {
			@Override
			public void onSuccess(UtilisateurGrpModel arg0) {
				...
				if (arg0.getBadmin() != null && arg0.getBadmin() == true) {
					...
					service.findAll(new AsyncCallbackWithErrorResolution<List<ChantierModel>>() {
						@Override
					...
					service.findChantierByUser(arg0.getId(), new AsyncCallbackWithErrorResolution<List<ChantierModel>>() {
						@Override
						public void onSuccess(List<ChantierModel> arg0) {
							storeChantier.add(arg0);
						}
	...
	protected void onRender(Element parent, int index) {
		...
		mainContent.setWidth(GuiUtil.getScreenWidth() - 30);
		...
		GridCellRenderer<ChantierModel> deleteBtn = new GridCellRenderer<ChantierModel>() {
			...
			@Override
			public Object render(final ChantierModel model, String property, ColumnData config, int rowIndex, int colIndex,
					final ListStore<ChantierModel> store, Grid<ChantierModel> grid) {
				layoutcell = new LayoutContainer();
				layoutcell.setStyleAttribute("align", "center");
					...
					public void handleEvent(MessageBoxEvent be) {
						...
						if (Dialog.OK.equals(btt.getItemId())) {
							if (storeChantier == null) {
								...
								service.deleteChantier(model.getId(), new AsyncCallbackWithErrorResolution<Void>() {
									@Override
				...
				img2.addClickHandler(new ClickHandler() {
					@Override
					public void onClick(ClickEvent event) {
						serviceRole.findRoleById(model.getId(), user.getId(), new AsyncCallbackWithErrorResolution<RoleModel>() {
							@Override
							public void onSuccess(RoleModel role) {
								if (role != null && role.getBcontributeur() == false && role.getBlecteur() == false && user.getBadmin() == false) {
									...
									CookieUtil.setCurrentChantier(model.getId().toString());
									CookieUtil.setCurrentUser(user.getId().toString());
									// CookieUtil.setCurrentRole(model.getId().toString());
									navigation.getContext().setCurrentChantier(model);
									navigation.getContext().setRoleModel(role);
									navigation.getContext().setUserModel(user);
									History.newItem(model.getId().toString());
		...
		column = new ColumnConfig("supprimer", "", 85);
		...
		txtChantier.setWidth(250);
		txtChantier.setBorders(true);
		txtChantier.setStyleAttribute("border-color", "#938E8E");
		...
		fsNouveauxChantier.add(txtChantier);
		// fsNouveauxChantier.add(txtChantier, new com.extjs.gxt.ui.client.widget.layout.ColumnData(50.0));
		...
		btnCreer.setEnabled(false);
		Html html = new Html("&nbsp;&nbsp;&nbsp;");
		fsNouveauxChantier.add(html, new com.extjs.gxt.ui.client.widget.layout.ColumnData(10.0));
		...
		setDefaultBackgroundColor(fsNouveauxChantier);
		add(mainContent);
	}
}
