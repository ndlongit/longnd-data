...
public class TransfertppBreadcumbPanel extends AbstractPanel {
	...
	public void loadPanel(ChantierModel chantierModel, final Integer idTransfertpp) {
		serviceChantier.findChantierById(chantierModel.getId(), new AsyncCallback<ChantierModel>() {
			...
			public void onSuccess(final ChantierModel c) {
				serviceRefTransfert.findById(idTransfertpp, new AsyncCallback<SimpleDto>() {
					...
					public void onSuccess(SimpleDto arg0) {
						chantier.setHTML("Chantier:" + c.getNom() + "<label style='color:black'>&nbsp;>Fiche de Transfert PP: " + arg0.getLabel() + "</label>");
