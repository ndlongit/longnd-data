...
public class ClientAuthenticationServiceImpl extends DependencyInjectionRemoteServiceServlet implements ClientAuthenticationService {
    ...
    public UserModel login(String userName, String passWord) {
        ...
        User user = this.domUserService.findUserByUserName(userName, null);
        try {
            if (user != null) {
                if (this.isValidLogin(passWord, user)) {
                    ...
                    info = this.dozerMapper.map(user, UserModel.class);
                } else {
    ...
    public UserModel ssoLogin() {
        ...
        LOGGER.info("===========REMOTE USER: " + this.getLogonedUserName(remoteUser));
        LOGGER.info("===========REMOTE DOMAIN: " + this.getLogonedDomain(remoteUser));
        LOGGER.info("===========REQUEST INFO:" + attr.getRequest().getMethod() + "--" + attr.getRequest().getContextPath());

        if (remoteUser != null) {
            user = this.domUserService.findUserByUserName(this.getLogonedUserName(remoteUser), this.getLogonedDomain(remoteUser));
            if (user != null) {
