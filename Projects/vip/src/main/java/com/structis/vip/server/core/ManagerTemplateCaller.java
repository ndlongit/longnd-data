...
import com.structis.vip.server.mapper.ExceptionMapper;
...
public class ManagerTemplateCaller implements ManagerTemplateIfc {
     ...
     *            l'appel de manager
     * @param param
     *            les parametres des appel de manager avant le mapping
    ...
    public Object callManager(ManagerCallBack callBack, Object... param) {
        ...
        try {
            ...
            for (int i = 0; i < param.length; i++) {
                mappedParam[i] = this.modelBeanMapper.map(param[i]);
            }
            if (logger.isDebugEnabled()) {
            ...
            // Mapping output
            result = this.modelBeanMapper.map(resultTemp);
            if (logger.isDebugEnabled()) {
