...
/*	public final static HashMap<String, Method> mapValue = new HashMap<String, Method>();
	
	static {
		 try {
			  Integer[] input={new Integer(2),new Integer(6)};
		      Class delegationModelClass = Class.forName("DelegationModel");
		      Class[] parametor = new Class[1];
		      parametor[0]=Integer.TYPE;
		      mapValue.put(DEL_ID, delegationModelClass.getMethod("getId",parametor));
		      Method method = mapValue.get(DEL_ID);
		      String output = (String)method.invoke(new DelegationModel(), input);
