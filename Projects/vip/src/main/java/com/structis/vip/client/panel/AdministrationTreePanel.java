...
//					btnAjouter.setEnabled(true);
	...
	private void initTreePanel() {
			...
			protected void load(Object loadConfig, AsyncCallback<List<PerimetreTreeModel>> callback) {
				...
				if (model == null) {
					...
					if (isAdminForAllPerimetre) {
						clientEntiteService.getTreeModelById(
								SessionServiceImpl.getInstance().getEntiteContext().getEntId(), SessionServiceImpl
										.getInstance().getUserContext().getUserRoles(), callback);
					} else {
						clientPerimetreService.getTreeModelById(SessionServiceImpl.getInstance().getPerimetreContext().getPerId(),
								SessionServiceImpl.getInstance().getUserContext().getUserRoles(), callback);
					}
				} else {
					model.setName(SafeHtmlUtils.htmlEscape(model.getName()));
					clientPerimetreService.getTreeModelByParent(SessionServiceImpl.getInstance().getEntiteContext()
							.getEntId(), SessionServiceImpl.getInstance().getUserContext().getUserRoles(), model,
							callback);
				}
			}
		};
		...
		tree.setId(ConstantClient.ADMIN_TREE_ID);
			...
			public void componentSelected(ButtonEvent ce) {
				if (!AppUtil.checkToShowWarningInAdminEditMode(true)) {
					...
					if (node != null) {
						if (node.getIsUoAdmin()) {
							if (ConstantClient.ENTITE_ID_IS_ETDE.equals(node.getEntiteId())) {
								AppUtil.showConfirmMessageBox(messages.perimetreaddconfirm(node.getName()), messages.commonContinueButton(), messages.commonAnnulerButton(), new Listener<MessageBoxEvent>() {
									@Override
						...
						public void onSuccess(Integer arg0) {
							if (arg0 > 0) { // if has at least a delegation or controle : cannot delete the perimetre
								...
								clientPerimetreService.hasReferenceInUserCollaborateur(node.getPerId(), new AsyncCallback<Integer>() {
									public void onSuccess(Integer arg0) {
										if (arg0 > 0) { // if has at least a user or collaborateur : show warning to update referenece to null
//											DeletePerimetreDialog dlg = new DeletePerimetreDialog(bus,node.getPerId(),arg0);
//											dlg.show();
											AppUtil.showConfirmMessageBox(messages.perimetredeletemanuallynodelegation(), new Listener<MessageBoxEvent>() {
												@Override
												public void handleEvent(MessageBoxEvent be) {
													if (be.getButtonClicked().getText().equalsIgnoreCase(messages.commonDialogOuiButton()) ) {
														...
														clientPerimetreService.clearReferenceToPerimetreInUserCollaborateur(node.getPerId(), new AsyncCallback<Void>() {
...
//					});
	...
	private void deletePerimetre(
			final PerimetreTreeModel node) {
		clientPerimetreService.deleteById(node.getPerId(), new AsyncCallback<Boolean>() {
