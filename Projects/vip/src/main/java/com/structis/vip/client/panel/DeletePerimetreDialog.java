...
public class DeletePerimetreDialog extends Window {
	...
	public void initData() {
		int numOfGrid = 0;
		if ((errorType & SharedConstant.REFER_DELEGATION) > 0) { //
			delegationPanel = createDelegationPanel();
			add(delegationPanel);
			numOfGrid++;
			ClientDelegationServiceAsync.Util.getInstance().findByPerimetre(perId, new AsyncCallback<List<DelegationModel>>() {
		...
		if ((errorType & SharedConstant.REFER_PERIMETRE) > 0) { //
			perimetrePanel = createPerimetrePanel();
			add(perimetrePanel);
			numOfGrid++;
			ClientPerimetreServiceAsync.Util.getInstance().findByPerimetreParent(perId, new AsyncCallback<List<PerimetreModel>>() {
		...
		if ((errorType & SharedConstant.REFER_COLLABORATEUR) > 0) { //
			collaborateurPanel = createCollaborateurPanel();
			add(collaborateurPanel);
			numOfGrid++;
			ClientCollaborateurServiceAsync.Util.getInstance().findByPerimetre(perId, new AsyncCallback<List<CollaborateurModel>>() {
		...
		if ((errorType & SharedConstant.REFER_USER) > 0) { //
			userPanel = createUserPanel();
			add(userPanel);
			numOfGrid++;
			ClientUserServiceAsync.Util.getInstance().findByPerimetre(perId, new AsyncCallback<List<UserModel>>() {
		...
		if ((errorType & SharedConstant.REFER_CONTROL) > 0) { //
			controlPanel = createControlPanel();
			add(controlPanel);
			numOfGrid++;
			ClientControlServiceAsync.Util.getInstance().findByPerimetre(perId, new AsyncCallback<List<ControlModel>>() {
		...
		setHeight(HEIGHT + numOfGrid * GRID_HEIGHT);
	}
