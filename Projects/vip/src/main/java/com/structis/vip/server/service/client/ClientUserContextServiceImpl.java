...
public class ClientUserContextServiceImpl extends DependencyInjectionRemoteServiceServlet implements ClientUserContextService {
	...
	.getLogger(ClientUserContextServiceImpl.class);
	...
	public UserModel checkUserByKerberosSSO() throws AppException {
		...
		if (request.getUserPrincipal() != null
			...
			LOGGER.debug("Login de l'utilisateur récupèré avec succès via le SSO Kerberos - "
					+ userLogin);
			return this.findUserOnDomainBC(userLogin.substring(0,
							userLogin.indexOf("@")).toLowerCase());
		}
	...
	@Override
	public UserModel findUserOnDomainBC(String login)
			throws AppException {
		UserModel clientUserContext;
		User domUser = domUserService.findUserByUserName(login, null);
		if (domUser != null) {
			LOGGER.debug("Utilisateur trouvé, initialisation des rôles");
			clientUserContext = this.mapToModel(domUser);
			return setUserContext(clientUserContext);
		}
	...
	private UserModel setUserContext(UserModel userContext) throws AppException {
		...
		if (userContext != null) {
			if (!sessionService.openSession(httpSession, userContext)) {
				throw AppExceptionFactory.getInstance().getAppException(
						AppExceptionType.USER_ALREADY_CONNECTED);
			}
	...
	private UserModel mapToModel(final User user) {
		return dozerMapper.map(user, UserModel.class);
	}

	@Override
	public void disconnectUser(UserModel userContext) {
		sessionService.killSession(userContext);
