...
//				}
		...
		bus.addHandler(SynETDEEvent.getType(), new SynETDEHandler() {
			...
			public void onLoadAction(SynETDEEvent event) {
				...
				clientSyncService.syncRUBISWithItems(SessionServiceImpl.getInstance().getEntiteContext().getEntId(),
						event.getModels(),
	...
	private void initData() {		
		...
		pagingConfig.setLimit(ConstantClient.DEFAULT_PAGE_SIZE_50);
...
//					}					
					...
					btnAddPerimetreDelegataire.setEnabled(se.getSelectedItem().getIsDelegataire() != null && se.getSelectedItem().getIsDelegataire() > 0);
				} else {
...
//		topToolBar.add(new LabelToolItem("Sortie"));//tdo
		...
		GridCellRenderer<CollaborateurModel> delegantRender = new GridCellRenderer<CollaborateurModel>() {
			...
			@Override
			public Object render(final CollaborateurModel model, String property,
...
//			}
	...
	private void applyPerimetreForDelegant(
			final CollaborateurModel model, int isDelegant, int isAddOrReplace) {		
		List<PerimetreTreeModel> selectedPerimetres = new ArrayList<PerimetreTreeModel>();
		TreePanel<PerimetreTreeModel> perimetreTree = getAdminTree();
		if (perimetreTree != null && model != null) {
