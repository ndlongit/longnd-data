...
//		config.add(cfAction);
	...
	public void initData() {
		...
		clientDelegationModelServiceAsync.getAllDelegationModelsByEntite(SessionServiceImpl.getInstance().getEntiteContext(), new AsyncCallback<List<DelegationMdlModel>>() {
			@Override
	...
	private Object buildActionColumn(final GroupDelegationMdlModel model) {
		final Button btn = new Button();
		btn.setText(messages.action());
		Menu menu = new Menu();

		MenuItem menuItemEdit = new MenuItem(messages.commonModifierButton());
		menuItemEdit.addSelectionListener(new ActionMenu(1, model));

		menu.add(menuItemEdit);

		MenuItem menuItemDelete = new MenuItem(messages.commonSupprimer());
		menuItemDelete.addSelectionListener(new ActionMenu(2, model));
