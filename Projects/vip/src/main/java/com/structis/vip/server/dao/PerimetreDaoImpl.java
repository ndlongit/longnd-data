...
//	private static final String PERIMETRE_ROOT_ENTITY = " p.perId != 'ETDE' AND p.perId != 'BYEFE'";
	...
	public List<Perimetre> getTreeModelByParent(String entiteId, String perimetreId) {
		...
		if (perimetreId == null) {
			...
			String sql = " from Perimetre p " + " where p.entite.entId = :idEntite AND p.parent.perId = :idPerimetre "
					+ " order by p.name";
			Query query = getEntityManager().createQuery(sql);
			query.setParameter("idEntite", entiteId);
			query.setParameter("idPerimetre", perimetreId);
	...
	public boolean isPerimetreChildOf(String compareId, String parentPer) {		
		 try{		 
		 Query query = getEntityManager().createNativeQuery("select dbo.ufn_GetIsPerimetreChildOf(?,?)");  
	...
	public Integer hasReferenceInDelegationOrControlOrPerimetre(String perId) {
		Integer ret = hasReferenceInDelegation(perId);		
		ret = ret | hasReferenceInControl(perId);
		ret = ret | hasReferenceInPerimetre(perId);
		return ret;
	}
	...
	public Integer hasReferenceInUserCollaborateur(String perId) {
		Integer ret = hasReferenceInUser(perId);		
		ret = ret | hasReferenceInCollaborateur(perId);		
		return ret;
	}
	...
	private Integer hasReferenceInUser(String perId) {	
		...
		Long result = (Long)query.getSingleResult();
		if (result > 0) return Constants.REFER_USER;
	...
	public void clearReferenceToPerimetreInUserCollaborateur(String perId) {
		clearReferenceToPerimetreInUser(perId);		
		clearReferenceToPerimetreInCollaborateur(perId);
	}
	...
	public List<Perimetre> findAndDeleteOrphanPerimetresNotInArgosByEntite(String entiteId,String parentId, 
		...
		return findUsedPerimetresNotInArgosByEntite(entiteId, parentId, idsInArgos);
	}

	
	private void deleteOrphanPerimetresNotInArgosByEntite(String entiteId,String parentId, 
			List<String> idsInArgos) {	
		StringBuilder sql = new StringBuilder();
		
		sql.append(" from Perimetre p where p.entite.entId = :entId AND p.argosName is not null ");
		if (idsInArgos != null && idsInArgos.size() > 0) {
			sql.append(" AND p.perId not in (:perIds) ");
		}
		List<String> ids = null;
		if (parentId != null) {			
			ids = getAllChildrenIdFrom(entiteId, parentId);
			if (ids != null && ids.size() > 0) {
				sql.append(" AND p.perId in (:idsChildren) ");
			}
		}
		sql.append(" AND p.perId not in (select distinct u.perimetre.perId from User u where u.entite.entId = :entId and u.perimetre is not null )")
		   .append(" AND p.perId not in (select distinct r.perimetre.perId from UserRole r where r.perimetre is not null )")
		   .append(" AND p.perId not in (select distinct d.perimeter.perId from Delegation d where d.entite.entId = :entId and d.perimeter is not null)")
		   .append(" AND p.perId not in (select distinct c.perimetre.perId from Control c where c.perimetre is not null)")
		   .append(" AND p.perId not in (select distinct e.perimetreDelegant.perId from Collaborateur e where e.entite.entId = :entId and e.perimetreDelegant is not null)")
		   .append(" AND p.perId not in (select distinct f.perimetreDelegataire.perId from Collaborateur f where f.entite.entId = :entId and f.perimetreDelegataire is not null)")
		   .append(" AND p.perId not in (select distinct g.perimetre.perId from DelegatairePerimetre g)")
		   .append(" AND p.perId not in (select distinct h.perimetre.perId from DelegantPerimetre h)")
		   .append(" AND p.perId not in (select distinct p1.parent.perId from Perimetre p1 where p1.parent is not null)");
	...
	private List<String> getAllChildrenIdFrom(String entiteId, String perimetreId) {
		...
		if (resultList != null && resultList.size() >0) {
			...
			for (String id: resultList) {
				finalResultList.addAll(getAllChildrenIdFrom(entiteId, id));
			}
		}
		return resultList;
	...
	private List<Perimetre> findUsedPerimetresNotInArgosByEntite(
			String entiteId, String parentId, List<String> idsInArgos) {	
		StringBuilder sql = new StringBuilder();
