...
public class ClientSyncServiceImpl extends DependencyInjectionRemoteServiceServlet implements ClientSyncService {
	...
	public List<CollaborateurModel> syncRUBIS(String entityId) {
		...
		List<SynETDEModel> codes = getRubsiCodesNameEtde();
		if (codes.isEmpty() == false) {
			for (int i=0; i<codes.size(); i++) {
				if (codes.get(i) != null) {
					ret.addAll(syncRUBISWithCode(entityId, codes.get(i).getCode()));
				}
			}
		}
...
//		}
		...
		Credentials creds = new UsernamePasswordCredentials(CatalinaPropertiesUtil.getRubisUsername(),
				CatalinaPropertiesUtil.getRubisPassword());
		...
		try {
			...
			if (!handler.getCollaborateurs().isEmpty()) {
				...
				for (Collaborateur item : handler.getCollaborateurs()) {
					...
					collaborateur = domCollaborateurService.findByIdBycn(item.getIdBycn());
					address = getAddress(item.getIdBycn());
					
					if (address != null) {
						item.setAddress(getAddress(address));
					}
					
					try {
						if (collaborateur == null) {
							domCollaborateurService.insert(item);
						} else {
							...
							domCollaborateurService.update(item);
						}
					} catch (Exception e) {
	...
	private String getAddress(final AddressModel address) {
		...
		if (address.getPays() != null) {
			PayCode payCode = domPayCodeService.findByCode(address.getPays());
			if (payCode != null) {
	...
	@Override
	public List<CollaborateurModel> syncRUBISWithItems(String entId,
			List<SynETDEModel> models) {
		List<CollaborateurModel> ret = new ArrayList<CollaborateurModel>();
		...
		if (models != null && models.isEmpty() == false) {
			for (SynETDEModel synETDEModel : models) {
				...
				ret.addAll(syncRUBISWithCode(entId, synETDEModel.getCode()));
			}
		}
		return null;
	}
