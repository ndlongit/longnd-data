...
public class ClientUserServiceImpl extends DependencyInjectionRemoteServiceServlet implements ClientUserService {
    ...
    public List<UserModel> findUsers() {
        ManagerCallBack callBack = new ManagerCallBack() {
            ...
            public Object execute(Object... inputs) {
                return ClientUserServiceImpl.this.domUserService.findUsers();
            }
        };
        return (List<UserModel>) this.callManager(callBack);
    }
    ...
    public List<UserModel> findUsersByEntite(final String entiteId, UserModel userModel) {
        ...
        for (User user : this.domUserService.findUsersByEntite(entiteId)) {
            UserModel usr = this.findUserById(user.getId());
            usr.setRoles();
            if (!usr.isSuperUser()) {
                ...
                if (userModel.isApplicationAdmin()) {
                    ...
                    if (this.isChild(usr.getPerimetre(), userModel.getPerimetre())) {
                        if (usr.isUoAdmin()) {
    ...
    private boolean isChild(PerimetreModel perimetre, PerimetreModel perimetre2) {
        ...
        return this.domPerimetreService.isPerimetreChild(perimetre.getPerId(), perimetre2.getPerId());
    }

    @Override
    public UserModel findUserById(int id) {
        User user = this.domUserService.findUserById(id);
        return (UserModel) this.modelBeanMapper.map(user);
    }

    @Override
    public UserModel findUserByUserName(String userName, String domain) {
        User user = this.domUserService.findUserByUserName(userName, domain);
        return (UserModel) this.modelBeanMapper.map(user);
    }

    @Override
    public UserModel insert(UserModel userModel) throws UserException {
        User user = (User) this.modelBeanMapper.map(userModel);
        user = this.domUserService.insert(user);
        return (UserModel) this.modelBeanMapper.map(user);
    }
    ...
    public UserModel update(UserModel userModel) throws UserException {
        User user = (User) this.modelBeanMapper.map(userModel);
        user = this.domUserService.update(user);
        return (UserModel) this.modelBeanMapper.map(user);
    }
    ...
    public Boolean delete(UserModel userModel) {
        User user = (User) this.modelBeanMapper.map(userModel);
        return this.domUserService.deleteWithRoles(user);
    }

    @Override
    public UserModel getAuthorization(final String name, final Integer domainId, final String password) throws UserException {
        User user = this.domUserService.getAuthorization(name, domainId, password);
        return (UserModel) this.modelBeanMapper.map(user);
        // if (user == null) {
    ...
    public ExceptionType changePassword(Integer userId, String value, String newValue) {
        return this.domUserService.changePassword(userId, value, newValue);
    }

    @Override
    public UserModel updateNoRoles(UserModel userModel) {
        User user = (User) this.modelBeanMapper.map(userModel);
        user = this.domUserService.updateNoRoles(user);
        return (UserModel) this.modelBeanMapper.map(user);
    }
    ...
    public PagingLoadResult<UserModel> findUsersByEntiteRemote(PagingLoadConfig config, String entId, UserModel userModel) {
        ...
        all = this.findUsersByEntite(entId, userModel);
    ...
    public List<UserModel> findByPerimetre(final String perId) {
        ManagerCallBack callBack = new ManagerCallBack() {
            ...
            @Override
            public Object execute(Object... inputs) {
                return ClientUserServiceImpl.this.domUserService.findByPerimetre(perId);
            }
        };
        return (List<UserModel>) this.callManager(callBack);
    }
