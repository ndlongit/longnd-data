...
public class DomDocumentModelServiceImpl extends GenericEntityServiceImpl<DocumentMdl, Integer>
		implements DomDocumentModelService {
	...
	public List<DocumentMdl> getAllDocumentModels() {		
		return documentMdlDao.getAllDocumentModels();
	}

	@Override
	public List<DocumentMdl> getDocumentModelsByLanguage(Integer langId) {
		return documentMdlDao.getDocumentModelsByLanguage(langId);		
	}

	@Override
	public List<DocumentMdl> getDocumentModelsByLanguageAndType(
			Integer langId, String type) {
		return documentMdlDao.getDocumentModelsByLanguageAndType(langId, type);
	}
	...
	public List<DomDel> getDocumentsByDelegation(Integer delegationId) {
		return documentMdlDao.getDocumentsByDelegation(delegationId);
	}

	@Override
	public List<DomDel> getDocumentsByDelegationAndLanguage(
			Integer delegationId, Integer languageId) {
		return documentMdlDao.getDocumentsByDelegationAndLanguage(delegationId, languageId);
	}

	@Override
	public List<DomDel> getDocumentsByDelegationLanguageAndType(
			Integer delegationId, Integer languageId, String type) {
		return documentMdlDao.getDocumentsByDelegationLanguageAndType(delegationId, languageId, type);
	}

	@Override
	public List<DocumentMdl> getDocumentModelsByEntiteId(String entId) {		
		return documentMdlDao.getAllDocumentModelsByEntiteId(entId);
	}
	...
	public Boolean deleteDocument(DomDel document, String path) {
		...
		if (document.getDelegation() != null) {
			...
			if (status.getId() == DelegationConstants.DEL_STATUS_VENIR || status.getId() == DelegationConstants.DEL_STATUS_SIGNEE) {
				...
				delegationDao.update(document.getDelegation());
				if (document.getDelegation().getParent() != null) {
					...
					if (currentDate.before(startDate)) {
						document.getDelegation().getParent().setDelegationStatus(domDelegationStatusService.findById(DelegationConstants.DEL_STATUS_VENIR));
					} else if(currentDate.after(startDate)) {
						document.getDelegation().getParent().setDelegationStatus(domDelegationStatusService.findById(DelegationConstants.DEL_STATUS_SIGNEE));
					} else {
						document.getDelegation().getParent().setDelegationStatus(domDelegationStatusService.findById(DelegationConstants.DEL_STATUS_SIGNEE));
					}
					delegationDao.update(document.getDelegation().getParent());
				}
			}
		}
		return true;	
	...
	public DocumentMdl insert(DocumentMdl document) {
		return documentMdlDao.insert(document);
	}

	@Override
	public DocumentMdl update(DocumentMdl doc) {
		return documentMdlDao.update(doc);
	}

	@Override
	public List<DocumentMdl> findByName(String name, String entId) {
		return documentMdlDao.findByName(name, entId);
	}
